...
impl ApiKey {
    //START-----tobase64 Function (Implementation)
    pub fn to_base64(&self) -> String {
        base64::encode(self.0.as_slice())
    }
    //END-----tobase64 Function (Implementation)

    //START-----into_inner Function (Implementation)
    pub fn into_inner(self) -> Vec<u8> {
        self.0
    }
    //END-----into_inner Function (Implementation)
}
...
pub async fn new_api_key(database: &State<AppDatabase>) -> Result<Json<&str>, ApiError> {
    ...
    println!("Ape Key: {}", api_key.to_base64());
