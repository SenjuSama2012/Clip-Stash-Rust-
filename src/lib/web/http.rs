...
//START-----get_clip Function (Implmentation)
...
pub async fn get_clip(
    ...
    match action::get_clip(shortcode.clone().into(), database.get_pool()).await {
        ...
        Err(e) => match e {
            ...
            _ => Err(PageError::Internal("Server Serror".to_owned())),
        },
    }
}
//END-----get_clip Function (Implmentation)

//START-----submit_clip Function (Implmentation)
...
//END-----submit_clip Function (Implmentation)
    ...
    match action::get_clip(req, database.get_pool()).await {
        ...
        Err(e) => match e {
            ServiceError::PermissionError(msg) => {
                Ok(status::Custom(Status::Unauthorized, "".to_owned()))
            }
...
pub mod test {
    ...
    fn error_on_missing_clip() {
        let client = client();
        let response = client.get("/clip/asdfasdf").dispatch();
